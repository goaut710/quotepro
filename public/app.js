...
async function downloadPDF() {
  ...
  tmpl.innerHTML = `
  <div style="font-family:'Space Grotesk',sans-serif;color:#111;background:#fff;padding:48px;max-width:794px">
    <!-- HEADER -->
    <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:36px;border-bottom:2px solid #e63329;padding-bottom:24px">
      <div>
        ${logoDataURL ? `<img src="${logoDataURL}" style="height:56px;object-fit:contain;margin-bottom:8px;display:block"/>` : ''}
        <div style="font-size:20px;font-weight:700;letter-spacing:-0.5px">${esc(company)}</div>
        <div style="font-size:12px;color:#555;margin-top:4px">${esc(address)}</div>
        <div style="font-size:12px;color:#555">${esc(phone)} ${email ? '· ' + esc(email) : ''}</div>
      </div>
      <div style="text-align:right">
        <div style="font-size:30px;font-weight:800;color:#e63329;letter-spacing:-1px">COTIZACIÓN</div>
        <div style="font-size:22px;font-weight:700;font-family:monospace;margin-top:4px">${qNum}</div>
        <div style="font-size:13px;color:#555;margin-top:4px">Fecha: ${formatDate(qDate)}</div>
        <div style="display:inline-block;margin-top:8px;background:${status==='aprobada'?'#d1fae5':status==='rechazada'?'#fee2e2':'#fef3c7'};color:${status==='aprobada'?'#065f46':status==='rechazada'?'#991b1b':'#92400e'};padding:3px 12px;border-radius:20px;font-size:11px;font-weight:700;text-transform:uppercase">${status}</div>
      </div>
    </div>

    <!-- CLIENT -->
    <div style="background:#f8f8f8;border-radius:8px;padding:16px 20px;margin-bottom:28px">
      <div style="font-size:10px;font-weight:700;letter-spacing:1.5px;color:#888;margin-bottom:10px">DATOS DEL CLIENTE</div>
      <div style="font-size:16px;font-weight:700">${esc(client)}</div>
      ${cPhone ? `<div style="font-size:13px;color:#555;margin-top:3px">${esc(cPhone)}</div>` : ''}
      ${cEmail ? `<div style="font-size:13px;color:#555">${esc(cEmail)}</div>` : ''}
    </div>
    ...
    <table style="width:100%;border-collapse:collapse;margin-bottom:24px;font-size:13px">
      <thead>
        <tr style="background:#111;color:#fff">
          <th style="padding:10px 12px;text-align:left;border-radius:4px 0 0 4px">Descripción</th>
          <th style="padding:10px 8px;text-align:center;width:60px">Cant.</th>
          <th style="padding:10px 8px;text-align:right;width:100px">Precio</th>
          <th style="padding:10px 8px;text-align:right;width:70px">Desc.</th>
          <th style="padding:10px 12px;text-align:right;width:110px;border-radius:0 4px 4px 0">Total</th>
        </tr>
      </thead>
      <tbody>
        ${items.map((i,idx) => `
        <tr style="background:${idx%2?'#f8f8f8':'#fff'}">
          <td style="padding:10px 12px;border-bottom:1px solid #eee">${esc(i.desc)}</td>
          <td style="padding:10px 8px;text-align:center;border-bottom:1px solid #eee">${i.qty}</td>
          <td style="padding:10px 8px;text-align:right;border-bottom:1px solid #eee;font-family:monospace">${fmt(i.price)}</td>
          <td style="padding:10px 8px;text-align:right;border-bottom:1px solid #eee">${i.disc||0}%</td>
          <td style="padding:10px 12px;text-align:right;border-bottom:1px solid #eee;font-family:monospace;font-weight:600">${fmt(i.total)}</td>
        </tr>`).join('')}
      </tbody>
    </table>
    ...
    <div style="display:flex;justify-content:flex-end;margin-bottom:32px">
      <div style="width:260px">
        ...
        <div style="display:flex;justify-content:space-between;padding:8px 0;border-bottom:1px solid #eee;font-size:14px">
          <span style="color:#555">IVA (${currentUser?.tax_rate??12}%)</span>
          <span style="font-family:monospace">${fmt(tax)}</span>
        </div>
        <div style="display:flex;justify-content:space-between;padding:12px 0;background:#e63329;color:#fff;margin:0 -8px;padding:10px 8px;border-radius:6px;font-size:17px;font-weight:700;margin-top:6px">
          <span>TOTAL</span>
    ...
    <!-- NOTES -->
    ${notes ? `<div style="background:#f8f8f8;border-radius:8px;padding:14px 18px;margin-bottom:32px;font-size:13px;color:#444">
      <div style="font-size:10px;font-weight:700;letter-spacing:1.5px;color:#888;margin-bottom:6px">NOTAS</div>
      ${esc(notes)}
    </div>` : ''}

    <!-- SIGNATURES -->
    <div style="display:flex;justify-content:space-around;margin-top:48px;padding-top:32px;border-top:1px solid #eee">
      <div style="text-align:center">
        <div style="width:180px;border-top:1.5px solid #333;margin:0 auto;padding-top:8px;font-size:12px;color:#555">Firma Cliente</div>
      </div>
      <div style="text-align:center">
        <div style="width:180px;border-top:1.5px solid #333;margin:0 auto;padding-top:8px;font-size:12px;color:#555">Firma Técnico / Vendedor</div>
      </div>
    </div>

    <div style="text-align:center;margin-top:32px;font-size:11px;color:#bbb">Generado con QuotePro · ${new Date().toLocaleString()}</div>
  </div>`;

  const { jsPDF } = window.jspdf;
  const pdf = new jsPDF({ format: 'a4', unit: 'px', hotfixes: ['px_scaling'] });

  await html2canvas(tmpl.firstElementChild, {
    scale: 2,
    useCORS: true,
    ...
    if (imgH <= pdfH) {
      ...
      // Multi-page split
      let yPos = 0;
      const pageH = Math.floor(canvas.height * (pdfH / imgH));
      let page = 0;
      while (yPos < canvas.height) {
        if (page > 0) pdf.addPage();
        const tempCanvas = document.createElement('canvas');
        tempCanvas.width  = canvas.width;
        tempCanvas.height = Math.min(pageH, canvas.height - yPos);
        const ctx = tempCanvas.getContext('2d');
        ctx.drawImage(canvas, 0, -yPos);
        const slice = tempCanvas.toDataURL('image/png');
        pdf.addImage(slice, 'PNG', 0, 0, pdfW, pdfH);
        yPos += pageH;
        page++;
      }
    }
